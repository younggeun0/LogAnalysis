...
public class Result extends JDialog {
	...
	public Result(SelectMenuEvt sme, SelectMenu sl) {
		...
		if (sme.getStart() == 0 && sme.getEnd() == 0) {
			bgr.add(new JLabel("6. " + (sme.getStart() + 1) + "~" + sme.getRequestNum() + " 라인 가장 빈도수가 높은 key와 횟수")).setBounds(80, 410, 250, 30);
			bgr.add(new JLabel(sme.getMostFrequentKey() + " : " + sme.getMapKey().get(sme.getMostFrequentKey()) + "번")).setBounds(380, 410, 200, 30);
		} else {
